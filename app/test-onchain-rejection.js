#!/usr/bin/env node
// Test script to create ON-CHAIN proof of limit order rejection
...
async function main() {
  ...
  console.log(C.bold(C.c("ON-CHAIN Rejection Test - X1 Testnet")));
  console.log(C.bold(C.c("=".repeat(60))));
  console.log("");
  ...
  console.log(C.y("This creates permanent on-chain proof of limit order rejection"));
  console.log("");

  // BUY YES with impossibly low limit - WILL FAIL
  console.log(C.bold("Submitting: BUY YES with price limit $0.01 (will be rejected)"));
  console.log("");

  const amount = 100_000; // 0.1 shares
  const priceLimit = 10_000; // $0.01 per share (way too low)

  const data2 = encodeTradeGuarded(SIDE_YES, ACTION_BUY, amount, priceLimit);
      ...
      if (txDetails && txDetails.meta) {
        ...
        if (txDetails.meta.logMessages) {
          txDetails.meta.logMessages.forEach(log => {
            if (log.includes("PriceLimitExceeded") || log.includes("LIMIT CHECK")) {
        ...
        console.log(C.g("showing that the limit order was rejected with PriceLimitExceeded"));
      }
    }
  } catch (err) {
  ...
  console.log("");
  console.log(C.bold(C.c("=".repeat(60))));
  console.log(C.bold(C.g("On-chain rejection test complete!")));
  console.log(C.bold(C.c("=".repeat(60))));
}
