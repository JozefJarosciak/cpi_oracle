#!/usr/bin/env node
// app/settlement_bot.js — Automated settlement bot for 1-hour market cycles
// Cycles: 1 min pre-market -> snapshot -> 59 mins active -> settle -> restart
// Markets start at :00 minutes past the hour (e.g., 01:00, 02:00, 03:00, etc.)
...
const AMM_SEED = Buffer.from("amm_btc_1h");  // v6: added market_end_time for time-based trading lockout
...
const ACTIVE_DURATION_MS = 59 * 60 * 1000;     // 59 minutes active (after snapshot)
const CYCLE_DURATION_MS = 60 * 60 * 1000;      // 60 minutes total cycle (:00 each hour)
...
// resetCostBasis removed - cost basis automatically scoped by cycle_id in trading_history
...
async function autoRedeemAllPositions(conn, kp, ammPda, vaultPda, snapshotPrice = null, settlePrice = null) {
  ...
  // Check vault_sol balance before redemptions
  const vaultSolBalanceBefore = await conn.getBalance(vaultPda);
  logInfo(`🏦 Vault SOL balance BEFORE redemptions: ${(vaultSolBalanceBefore/LAMPORTS_PER_XNT).toFixed(4)} XNT`);
  logInfo(`   (Reserve: 1.0000 XNT, Available: ${((vaultSolBalanceBefore - LAMPORTS_PER_XNT)/LAMPORTS_PER_XNT).toFixed(4)} XNT)`);
...
async function runCycle(conn, kp, ammPda, vaultPda) {
  ...
  try {
    ...
    // Reset volume for new market cycle
    // Cost basis no longer needs reset - automatically scoped by cycle_id in trading_history
    await resetVolume();
